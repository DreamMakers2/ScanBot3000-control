...
- `home z`
  - Request the host-side homing sequence. Requires the Z limit switch to be pressed (same behavior as the Z-axis home button on GPIO1).
- `tmcsettings <axis>`
...
- `led <axis> <index> <r> <g> <b> <bri>`
  - Set LED `<index>` (0..7) to RGB values with per-channel range 0..255 and a driver brightness byte 0..255 on the given axis. Inputs are clamped before transmission.
- `hi <axis>`
...
Homing is requested from the Z-axis controller home button (GPIO1); the Teensy runs the multi-axis sequence when the Z limit switch is already pressed. The `home z` command triggers the same sequence; use `stop` to abort.
